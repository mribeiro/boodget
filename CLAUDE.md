...
- **Loan**: A per-dossier loan, either `draft` (a study/what-if — principal + term + interest rate) or `active` (a real ongoing loan — remaining balance + end date + interest rate). Both compute a `monthly_payment` via the standard annuity formula and a `salary_pct` (payment ÷ per-loan `salary`, prefilled from the most recent cycle's salary). Active loans store an `end_date` (`YYYY-MM`, required — must resolve to `months_left ≥ 1`) instead of a directly-entered months-left figure; `months_left` is always derived fresh from `end_date` vs. the current date (`= (end_year×12+end_month) − (current_year×12+current_month) + 1`, clamped ≥ 0), never stored, so the user never has to update it every month. The `interest_rate` field is always the **TAN** (nominal rate that drives the calc), labeled "TAN" on draft loans and "Interest rate" on active loans — never the TAEG/APR, which is a separate, fees-inclusive figure. Status can be toggled both ways, preserving both sides' field values; demoting active → draft always clears the expense link and `end_date`; promoting draft → active always clears `down_payment`, `taeg`, and `opening_fee`. Draft loans may optionally set a `down_payment` (nullable, draft-only — 400 if set on an active loan), letting the create/edit form take a purchase price + down payment instead of a directly-typed principal; the amortization calc always uses `principal` (amount financed), and a computed `purchase_price` (`principal + down_payment`) is returned/shown whenever a down payment is set. Draft loans may also optionally set `taeg` (reference-only, draft-only, never used in the calc — just for comparing against the TAN) and `opening_fee` (draft-only, a one-off processing commission); draft loans also expose two computed totals over the full term — `total_interest` (`monthly_payment × term_months − principal`) and `total_amount_payable` (MTIC, a simplified estimate = `monthly_payment × term_months + opening_fee`; the real legal MTIC can include untracked charges like stamp duty or insurance). Active loans may optionally link to a `Fixed` expense template item to check budget coverage (green/red, with a `0.005` epsilon), and offer three ephemeral scenario calculators (downpayment → lower payment or shorter term + interest saved; target payment → lump sum needed; interest rate change → new payment and interest cost, same balance and term, colored red/green by whether it's worse or better). The Loans tab list shows a `KpiStrip` summary (active loans only): total monthly amount, total amount due, loans ongoing, and total % of salary (`total monthly amount ÷ latest_cycle_salary`, not a sum of each loan's own `salary_pct`). See `ai-spec/SPECIFICATION_LOANS.md`.
...
- **Last applied migration**: `029_replace_months_left_with_end_date_on_loans`. **Next id must be `030_...`**
