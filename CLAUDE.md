...
- **AI Advisor**: A per-dossier tab that sends a trimmed snapshot of the dossier (accounts, capital series, expense template, recent cycles, goals, loans, subscriptions, EF status, annual years, annual expense template, workbench, reference salary) to the Claude API (Anthropic) or the Gemini API (Google) — the provider is derived purely from the selected `ai_model` id's prefix (`claude-*` vs `gemini-*`); there is no separate stored provider setting, so it can never drift out of sync with the chosen model. Active subscriptions contribute their `monthly_cost` and, when linked, their `linked_distribution` name — the model is instructed to compare a distribution's total linked subscriptions against that distribution's budgeted value and flag it as a risk if exceeded. Loans (both draft and active) are included via `computeLoanValues`, factoring their monthly payment, budget coverage, and total interest into the health score/highlights/risks, and compared against the dossier's `reference_salary`/`loans_max_salary_pct` ceiling when both are set; draft loans are treated as hypothetical studies, not commitments. Active loans contribute `current_balance` (the projected live figure — the dated `remaining_balance` anchor is deliberately *not* sent, since reading it as today's debt would overstate what's owed) plus `balance_as_of`/`payments_made`/`term_from_anchor` so the model can weigh how stale a projection is, and `payments_tracked` so an active loan with no monthly expense assigned can be flagged as unbudgeted. The annual expense template (the recurring definition annual years are instantiated from) contributes a `total_monthly_avg` used to sanity-check a year's budgeted total and to factor recurring-but-not-yet-budgeted costs into capacity. Up to the 3 most-recently-updated Workbench snapshots contribute Must/Want/Save/leftover totals (via a server-side `summarizeWorkbenchData`, mirroring the frontend's `computeGlobalSummary`) — treated as the user's own targets/plans, not actuals, useful for flagging an unaffordable plan or a drift from `recent_cycles`. **Any new dossier-scoped feature with financial impact must be added to this context payload too** (see Documentation Maintenance rules above). An "Analyze dossier" button returns a structured assessment (health score 0–100, summary, highlights, improvements, risks) persisted per dossier (latest only, in `ai_analyses`); a chatbox answers questions with the same context (ephemeral, client-side history, buffered responses). Model is a per-dossier setting `ai_model` (default `claude-opus-5`), picked in the tab or in Settings from a dynamic catalog rather than a hardcoded model list — the whitelist is enforced **by family** (any `claude-haiku-*`/`claude-sonnet-*`/`claude-opus-*` id, or any `gemini-{version}-{pro,flash}` id; `fable`/`mythos`, Gemini's `-lite`/`-8b`/`-image`/`-tts`/`-audio`/`-embedding` variants, and the versionless `gemini-{pro,flash}-latest` aliases are not offered), so a dossier's chosen model never becomes invalid just because a newer version of that family ships. `GET /ai-advisor/available-models` returns the current picker options (latest known version per family across both providers, cached app-wide in `app_settings` under `ai_available_models`, seeded with baked-in defaults until the first refresh); a "Refresh" button next to both model pickers calls `POST /ai-advisor/refresh-models`, which queries the Claude API's `GET /v1/models` (paginated via `after_id`, latest by `created_at`) and the Gemini API's `GET /v1beta/models` (paginated via `pageToken`, filtered to `generateContent`-capable models; since Google's list carries no creation timestamp, "latest" per family is determined by parsing the version number out of the id and comparing numerically, preferring a stable release over a preview at equal version) and keeps only the best model per family — older versions are dropped from the *picker*, but never retroactively invalidate a dossier already set to one (`resolveAiConfig` accepts any family member). A provider whose key doesn't resolve is silently skipped (its families keep their previously-cached/default entries, reported in the response's `skipped` list) rather than failing the whole refresh, so a dossier with only one provider's key can still refresh that half of the catalog; the endpoint 503s only when *neither* key resolves, and an upstream failure on an attempted provider is reported in `errors` rather than thrown unless it's the only provider attempted. It is not gated by `ai_enabled`, since the Settings picker must render even while the feature is off. Cost estimates are priced **per family** (`FAMILY_PRICING` in `ai-advisor.js`), not per exact model id, since neither provider's Models API exposes pricing and a family's rate is far more stable across version bumps than the id itself; Gemini's usage is normalized to the same shape Claude's is (`callGemini` folds `thoughtsTokenCount` into output tokens, since thinking is billed as output but excluded from `candidatesTokenCount` on thinking-by-default models, and leaves both cache-token fields at 0 since Gemini's implicit-cache discount isn't modeled — the estimate is deliberately conservative, never understated). The whole feature is gated per dossier by `ai_enabled` (default on) — configured in Dossier Settings → **AI Settings**, alongside the model picker and two optional per-dossier keys, `ai_api_key` (write-only, never returned by any endpoint; falls back to the operator-supplied `ANTHROPIC_API_KEY` env var when unset) and `ai_gemini_api_key` (same write-only treatment; falls back to `GEMINI_API_KEY`) — each key is only consulted when the dossier's selected model belongs to that provider. When disabled, the AI Advisor tab is not rendered at all (no AI reference anywhere in the dossier UI) and the backend independently 403s all endpoints. Every response shows a USD cost estimate from token usage × a hardcoded pricing table. A "Use your Claude or Gemini subscription instead" card (always visible, no API key required) lets the user copy or download the same context + analysis instructions as a self-contained prompt (`GET /ai-advisor/export-prompt`) to paste into claude.ai, gemini.google.com, or any other chat client, avoiding per-call API billing entirely. An "Additional context" textarea (`ai_user_context`, capped at 4000 chars) lets the user add free-text notes the raw numbers don't capture (e.g. explaining away a one-off expense spike); it's persisted per dossier and included in every analysis, chat turn, and exported prompt, with the model instructed to give it real weight. See `ai-spec/SPECIFICATION_AI_ADVISOR.md`.
...
│   │       ├── ai-advisor.js     # AI Advisor: dossier context builder, Claude + Gemini API calls, analysis + chat
...
| `dossiers` | Capital dossiers. `creator_id` FK → `users`. Holds `cycle_start_day` (default 25), `cycle_start_weekend_adjustment` (`none`/`previous_friday`/`next_monday`, default `none` — shifts a cycle's start off a Sat/Sun), three Glances warning thresholds (`capital_snapshot_warning_day` default 7, `next_cycle_warning_day` default 22, `previous_cycle_close_warning_day` default 25), two EF settings (`emergency_fund_months_multiplier` default 6, `emergency_fund_cycles_to_average` default 6), four Paperless fields (all nullable), `expense_notification_days_before` (default 1), `ai_enabled` (default `1` — gates the entire AI Advisor feature for the dossier), `ai_model` (default `claude-opus-5`; family whitelist — `claude-{haiku,sonnet,opus}-*` or `gemini-{version}-{pro,flash}` — enforced in settings PATCH and on dossier import, which coerces an out-of-family value to the default rather than storing it as-is), `ai_api_key` (nullable, secret — per-dossier Claude API key, write-only, never returned by any GET; falls back to the `ANTHROPIC_API_KEY` env var when unset), `ai_gemini_api_key` (nullable, secret — per-dossier Gemini API key, same write-only treatment; falls back to `GEMINI_API_KEY`), `ai_user_context` (nullable, user-authored free text ≤ 4000 chars, included in every AI Advisor prompt), `reference_salary` (nullable, manually-set — prefills new loans' `salary` and denominates the Loans tab's total % of salary; not derived from `expense_cycles`), `loans_max_salary_pct` (nullable, 0–100 — the Loans tab's red/amber/green threshold against `reference_salary`). |
...
- **Last applied migration**: `043_add_ai_gemini_api_key_to_dossiers`. **Next id must be `044_...`**
...
PATCH  /api/dossiers/:id/settings   { cycle_start_day?, cycle_start_weekend_adjustment?, capital_snapshot_warning_day?,
                                       ...
                                       ai_enabled?, ai_model?, ai_api_key?, ai_gemini_api_key?, ai_user_context?, reference_salary?, loans_max_salary_pct? }
...
GET    /api/dossiers/:id/ai-advisor/available-models   # { models: [{id, family, display_name}], updated_at } — latest known version per family, across both providers
POST   /api/dossiers/:id/ai-advisor/refresh-models      # re-fetches from Claude's GET /v1/models and Gemini's GET /v1beta/models; latest-per-family only;
                                                         # a provider with no resolvable key is skipped, not failed — see { models, updated_at, skipped, errors }
...
| `ANTHROPIC_API_KEY` | No | Fallback Anthropic API key for the AI Advisor, used by dossiers on a `claude-*` model that haven't set their own key in Settings → AI Settings. If no key resolves for the dossier's selected model's provider (neither dossier nor env var), the AI Advisor tab shows a setup hint and its POST endpoints return 503. |
| `GEMINI_API_KEY` | No | Fallback Google Gemini API key for the AI Advisor, used by dossiers on a `gemini-*` model that haven't set their own key (`ai_gemini_api_key`) in Settings → AI Settings. Only consulted when the dossier's selected `ai_model` is a Gemini model — same 503/setup-hint behaviour as `ANTHROPIC_API_KEY` otherwise. |
