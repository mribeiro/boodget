...
| `loans` | `name`, `status` (`draft`/`active`), `interest_rate` (annual %), `salary`, `principal`/`term_months`/`down_payment` (draft; `down_payment` nullable, cleared on promotion to active), `remaining_balance`/`months_left` (active), `expense_template_item_id` (nullable FK → `expense_template_items`, `ON DELETE SET NULL`, active only). No `position`/`updated_at`; list ordered `created_at ASC`. |
...
- **Last applied migration**: `027_add_down_payment_to_loans`. **Next id must be `028_...`**
...
POST   /api/dossiers/:id/loans   { name, status, interest_rate, salary?, principal?, term_months?, down_payment?, remaining_balance?, months_left?, expense_template_item_id? }
