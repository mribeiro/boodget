...
│   │   ├── utils/numbers.js, loanMath.js, carMath.js, cycleDates.js  # formatNumber/parseDecimalInput, loan amortization math, car energy-cost math, weekend-adjusted cycle date math
│   │   ├── pages/
...
- **Frontend** (`frontend/vitest.config.js`, separate from `vite.config.js` since that file's `vite-plugin-pwa` build-time config has no bearing on tests): co-located `*.test.js` next to the source file (e.g. `frontend/src/utils/loanMath.test.js`). Currently covers the pure-function utils (`numbers.js`, `loanMath.js`, `carMath.js`) only — no React Testing Library / component-test setup yet (deliberately deferred: no existing convention for mocking `services/api.js` or jsdom setup).
