...
- **AI Advisor**: A per-dossier tab that sends a trimmed snapshot of the dossier (accounts, capital series, expense template, recent cycles, goals, loans, EF status, annual years, reference salary) to the Claude API. Loans (both draft and active) are included via `computeLoanValues`, factoring their monthly payment, budget coverage, and total interest into the health score/highlights/risks; draft loans are treated as hypothetical studies, not commitments. An "Analyze dossier" button returns a structured assessment (health score 0–100, summary, highlights, improvements, risks) persisted per dossier (latest only, in `ai_analyses`); a chatbox answers questions with the same context (ephemeral, client-side history, buffered responses). Model is a per-dossier setting `ai_model` (default `claude-opus-4-8`; whitelist: haiku-4-5, sonnet-5, opus-4-8, fable-5) picked in the tab. Requires the operator-supplied `ANTHROPIC_API_KEY` env var; every response shows a USD cost estimate from token usage × a hardcoded pricing table. See `ai-spec/SPECIFICATION_AI_ADVISOR.md`.
...
- **Last applied migration**: `031_add_ai_advisor`. **Next id must be `032_...`**
