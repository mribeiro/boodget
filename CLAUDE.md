...
- **Last applied migration**: `040_add_enablebanking_redirect_uri_to_dossiers`. **Next id must be `041_...`**
...
PATCH  /api/dossiers/:id/settings   { cycle_start_day?, capital_snapshot_warning_day?,
                                       ...
                                       enablebanking_application_id?, enablebanking_private_key?, enablebanking_redirect_uri? }
...
| `ENABLE_BANKING_REDIRECT_URI` | No | Fallback full callback URL (e.g. `https://boodget.example.com/bank/callback`), used by dossiers that haven't set their own `enablebanking_redirect_uri` in Settings → Enable Banking Integration. Must exactly match what's whitelisted for the dossier's Enable Banking application. Not derived from request headers. |
