...
- **Annual Expense Template**: A per-dossier list of annual expenses (separate from the monthly expense template). Each entry has `name`, `value`, `classification`, `num_installments`, and a list of installment dates (`month`, `day`). Used by the Workbench to compute monthly averages (value / 12).
- **Annual Expense Year**: A per-dossier, per-calendar-year instance that copies items from the annual expense template at creation time and tracks actual payments throughout the year. Has a `carryover` field (amount carried from the prior year). Items are sorted by their first installment date ascending. The year summary card shows: Carryover, Accumulated (accounts), Contributed (distributions), Total budgeted, Total paid, Total expenses remaining, Amount left needed (`max(0, total_remaining − accumulated_accounts)`), Total raise needed (`max(0, total_budgeted − carryover)`, with projected annual distributions subtitle), Monthly average needed (`amount_left_needed / cycles_remaining` where `cycles_remaining` = number of cycles in the year whose start date is still in the future; label shows "N cycles left"; when 0 cycles remain shows `amount_left_needed` as-is; with projected monthly distributions subtitle), Needed this cycle.
...
- **Glances**: A read-only summary panel rendered above the tab bar in `DossierView`. Shows up to five cards (Emergency Fund, Capital, Current Cycle, Next Expense, Goals) with colour-coded states (neutral / amber / red). The Emergency Fund card only appears when the fund is underfunded. Clicking the **Current Cycle** card navigates to the relevant cycle's `CycleEditor` page (current cycle in normal state; previous cycle when the red "not closed" warning is active); falls back to the Monthly Expenses tab when no specific cycle is actionable. Clicking the **Next Expense** card navigates directly to the current cycle's `CycleEditor` page; falls back to the Monthly Expenses tab when no current cycle exists. Other cards navigate to their respective tabs. Three per-dossier warning day thresholds control when amber/red states activate. The Next Expense card shows the payment date as "Month Day" (e.g. "Mar 25") and, when overdue, displays an inline "Mark as paid" button that marks the item paid and refreshes the card without navigating away.
