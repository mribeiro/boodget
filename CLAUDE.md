...
- **Annual Expense Year**: A per-dossier, per-calendar-year instance that copies items from the annual expense template at creation time and tracks actual payments throughout the year. Has a `carryover` field (amount carried from the prior year). Items are sorted by their first installment date ascending. The year summary card shows: Carryover, Accumulated (accounts), Contributed (distributions), Total budgeted, Total paid, Total expenses remaining, Amount left needed (`max(0, total_remaining − accumulated_accounts)`), Total raise needed (`max(0, total_budgeted − carryover)`, with projected annual distributions subtitle), Monthly average needed (`total_raise_needed / 12`, with projected monthly distributions subtitle), Needed this cycle.
