...
│   │   ├── utils/numbers.js, loanMath.js, carMath.js, cycleDates.js, distributionGroups.js  # formatNumber/parseDecimalInput, loan amortization math, car energy-cost math, weekend-adjusted cycle date math, cycle distributions grouped by destination account
│   │   ├── pages/
...
| Table | Description |
|---|---|
| `users` | User accounts. `is_oidc=1` for SSO users. `avatar` (nullable TEXT) holds a self-uploaded profile picture as a `data:image/...;base64,...` data URL (client-resized to a 256×256 JPEG before upload); returned by `GET /auth/me` and `POST /auth/login`, never by `GET /users` (admin user list has no need for it). |
...
- **Last applied migration**: `045_create_car_adhoc_expenses`. **Next id must be `046_...`**
...
DELETE /api/users/:id                  # 409 if the user still owns any dossier (would cascade-delete them)
...
PATCH  /api/dossiers/:id/annual-expense-payments/:paymentId      { real_value?, paid? }  # 409 if linked to a closed cycle
...
| `[auth]` | Login success/failure, logout, password change, OIDC auto-creation, avatar updated/removed |
