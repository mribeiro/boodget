...
function kpiColor(highlight) {
  return highlight === 'success' ? 'var(--color-success)' :
...
export default function GoalDetail() {
  ...
  return (
    <div>
      ...
      {/* ── Stacked layout: hero → secondary KPIs → chart → contribution history ── */}
      <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)', marginBottom: '1.5rem' }}>

        {/* Hero: state + progress bar + headline numbers */}
        <div className="card card--flat">
          ...
          <div className="goal-hero-numbers">
            <div className="goal-hero-num">
              ...
              <div className="goal-hero-num-value">{formatEur(goal.target_value)}</div>
            </div>
            <div className="goal-hero-num">
              <div className="goal-hero-num-label">Progress</div>
              <div className="goal-hero-num-value" style={{ color: kpiColor(goal.state === 'completed' ? 'success' : 'neutral') }}>
                {formatEur(goal.total_current_progress)}
              </div>
            </div>
            <div className="goal-hero-num">
              <div className="goal-hero-num-label">Remaining</div>
              <div className="goal-hero-num-value" style={{ color: kpiColor(goal.remaining_amount > 0 && goal.state === 'active' ? 'neutral' : 'success') }}>
                {formatEur(goal.remaining_amount)}
              </div>
            </div>
          </div>
        </div>
        ...
        <KpiStrip defaultOpen items={[
          ...
          goal.anticipated_completion_date ? { label: 'Estimated done', value: formatYM(goal.anticipated_completion_date), highlight: 'success' } : null,
        ...
        {!isAdHoc && (
          <CollapsibleSection
            ...
            count={(goal.historical_contributions || []).length || undefined}
          >
