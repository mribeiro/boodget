...
export default function GoalsGlance({ goals, onClick, efStatus, onEfClick }) {
  ...
  const efBanner = efUnderfunded && (
    <div
      ...
      style={{ marginTop: 4, paddingTop: 6, borderTop: '1px solid var(--border-default)', cursor: onEfClick ? 'pointer' : 'default' }}
    >
      <div className="text-xs" style={{ color: 'var(--color-danger-text)', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 6 }}>
        <FontAwesomeIcon icon={faShieldHalved} />
        <span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>Emergency Fund: {formatEur(efStatus.deficit)} short</span>
