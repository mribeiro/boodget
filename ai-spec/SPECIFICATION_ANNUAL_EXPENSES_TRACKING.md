...
- A caption line below the headline numbers: "Starting point: {carryover} carried over from last year", with an inline edit pencil (opens the same carryover-editing modal). Carryover has no standalone row — it's framed as the year's starting point and lives inside the hero.
...
| Amount paid so far | Sum of all real_value where paid = true (formerly "Total paid"). Note line merges in the current-cycle due amount: "Due this cycle: X (covered)" / "(short Y)" / "Nothing due this cycle", coloured green/red — replaces the old separate "Needed this cycle" tile. |
| Remaining to pay | Total budgeted − Total paid (amber when > 0, green when 0); formerly "Total expenses remaining". |
| Raise needed for the year | max(0, Total budgeted − Carryover) — total amount to be raised over the year (formerly "Total raise needed"). Note spells out the formula ("Target X − carryover Y") plus projected annual distributions (selected distribution template values × 12), colour-coded green/amber vs. target. |
| Contributed via distributions | Sum of "done" distributions from cycles in this calendar year (`contributed_distributions`) — the *actual* counterpart to the *projected* distributions figures shown in the two tiles above/below. Note shows the amount still needed toward `total_raise_needed` and the configured monthly pace. Green when distributions alone have already covered `total_raise_needed`. |
| Estimated fully funded | Only shown when computable. The calendar month distributions alone are projected to fully cover `total_raise_needed`, mirroring Goals' `anticipated_completion_date` logic but scoped to the distributions channel: target = `total_raise_needed` (not the full budget — carryover is a lump sum already on hand, not something raised via distributions), progress = `contributed_distributions` (cumulative actual, from zero — not combined with carryover, since carryover isn't earned via distributions), pace = the same monthly-distributions-projected figure. Hidden if the pace is 0, if the target is already met, or if it wouldn't be reached before the year's remaining cycles run out. |
...
**Distributions funding progress chart**: below the KPI strip, a recharts line chart (mirroring `GoalDetail`'s chart, Section 8.2/8.3/8.4 of `SPECIFICATION_GOALS.md`) with three cumulative series — **Expected** (`monthly_dist_projected` accumulated once per cycle in the calendar year), **Real** (actual "done" distributions accumulated per cycle — the same computation as `contributed_distributions`, retained per-cycle instead of collapsed to one scalar), and a dashed **Projected** tail extrapolating from the current total through the year's remaining cycles at the same pace. A dashed `ReferenceLine` labelled "Estimated" marks the "Estimated fully funded" month when one is computable. Unlike Goals' chart, no anchoring/offset step is applied — Real at any cycle already **is** the running total of done distributions, with no independent live snapshot to reconcile against. The chart tracks the distributions contribution channel specifically, not the broader hero "raised to date" metric — it's hidden entirely (not shown as an all-projected line) when no distributions are selected for the year, or when the year has no cycles yet.

No pace-based "on track / behind" feasibility logic exists for the overall hero metric yet (unlike Goals' infeasibility check) — deferred to a future iteration. (The distributions-specific "Estimated fully funded" date above is a narrower, already-implemented analog scoped to the distributions channel only.)
