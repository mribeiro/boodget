...
- If one or more of a goal's linked accounts is **archived**, a warning is shown both in the goal list (a badge on the card) and in the goal detail view (a banner naming the archived account(s), with a one-click "Unlink now" action that removes just those accounts from the goal — every other linked account is left untouched). The archived account is no longer counted toward progress (see Section 5), but the link is only removed when the user explicitly unlinks it (from that banner, or by unchecking it in the edit form — see Section 2.1's account picker below).
- The **contributing accounts picker** in the edit form fetches accounts with `includeArchived=true`, but only surfaces an archived account if it is already linked to the goal (so it can be unchecked); archived accounts not already linked are not offered, since they're not expected to receive further funds.
