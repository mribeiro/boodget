...
- The Claude API is called with raw `fetch` (no SDK dependency): `POST https://api.anthropic.com/v1/messages` with `"stream": true`, headers `x-api-key`, `anthropic-version: 2023-06-01`, and a 120 s **idle** timeout on an `AbortController` — re-armed on every chunk received (keep-alive pings included), so only a stream that goes silent is abandoned ("stopped sending data for 120 seconds"), never one that is still producing a long answer. It used to be a fixed 180 s total, which cut off long Opus analyses after their tokens were already billed. The response body's Server-Sent Events are parsed by hand (`callClaudeStream` in `ai-advisor.js` — split on blank lines, read the `event:`/`data:` fields) rather than via an SDK helper, for the same no-new-dependency reason. No `thinking` parameter is sent (models use their own defaults).
...
- Uses **structured outputs** (`output_config.format` with a JSON schema) so the response always parses; `max_tokens` 32000 (streamed, so a high ceiling costs nothing unless used — only generated tokens are billed). Structured output is compatible with streaming — the JSON still arrives as ordinary `text_delta` chunks on one text content block, so it's only parsed once the stream completes (no partial-JSON parsing on the frontend).
...
- Ephemeral by design: history lives in a module-level store (`frontend/src/utils/aiAdvisorSession.js`), not in the backend and not in React component state, and the full history is re-sent each turn (`max_tokens` 8192 per reply). Nothing about a chat turn is written to the database.
