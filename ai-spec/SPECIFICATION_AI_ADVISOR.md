...
POST /api/dossiers/:id/ai-advisor/chat/start   { messages: [{role, content}] }
     → 202 { job_id }
     Validation: 1–40 messages, roles user/assistant, non-empty strings ≤ 8000 chars,
     must start and end with a user message (checked before the 503-not-configured check).
