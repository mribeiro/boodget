...
Each row is dated by calendar month/year, counting forward from the loan's **effective current period** (Section 4.1) rather than always "this month" — so if `day_of_payment` shows this month is already paid, the schedule's first row is next month, keeping its dates aligned with `months_left`'s own count instead of showing an already-paid month as still owed. `computeAmortizationSchedule(balance, ratePct, monthsLeft, payment, dayOfPayment)` returns the flat list of `{ year, month, interest, principal, balance }` rows; `groupScheduleByYear(schedule)` rolls that up into per-calendar-year buckets (`{ year, interest, principal, endBalance, months }`) for the UI (Section 9) to render as a collapsible year list that expands into its constituent months on demand, rather than rendering every payment at once.
