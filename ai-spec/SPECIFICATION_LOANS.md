...
| Field | Description |
|---|---|
| `monthly_payment` | Computed via Section 4 — `0` once matured (Section 4.1), same as the `months_left = 0` guard already produces |
...
| `remaining_interest` | `monthly_payment × months_left − remaining_balance` — `null` unless active, and also `null` when `is_matured` (would otherwise evaluate to a nonsensical negative figure, `0 × 0 − remaining_balance`). Interest still left to pay from now to payoff, using the loan's *current* balance/term — the forward-looking counterpart to `total_interest`'s backward-looking full-term figure |
