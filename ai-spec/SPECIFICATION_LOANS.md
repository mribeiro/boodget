...
- **Period → cycle**: cycles are loaded once and scanned in memory (a 30-year plan would otherwise be hundreds of round trips) via `backend/src/utils/cycleWindows.js`'s `loadCycleWindows`/`findCycleContainingDate` — the same shared DB-reading step Annual Expenses' `pull-annual-expenses` and Car Expenses use — matching on `actual_start_date <= due_date <= actual_end_date` with a recompute fallback for either date. A due date landing exactly on a `cycle_start_day` belongs to the cycle *starting* that day (inclusive start), and where two cycles genuinely overlap (`PATCH /cycles { resolve_overlap: 'ignore' }`) the first in `(year, month)` order wins.
...
- All numeric display uses `formatNumber`, and input parsing `parseDecimalInput` (amounts) / `parseRateInput` (rates) — never `Intl.NumberFormat` directly.
